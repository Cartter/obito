...
module.exports.run = async ({ client, message, channel, args, prefix, lang }) => {
    ...
    if (args[0] === undefined || args[0] === null) return channel.send(`${await client.utils.lang(lang, 'procurar')} EX \`${prefix}wikisamp OnPlayerConnect\``)

    request('https://sampwiki.blast.hk/wiki/' + args[0], async function (error, response, body) {
...
exports.help = {
    name: "wikisamp",
    aliases: ['wiki-samp', 'sampwiki'],
    description: "Faz uma busca no wikisamp de uma função",
    usage: 'wikisamp'
}